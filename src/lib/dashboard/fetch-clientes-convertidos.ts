...
// aceito, então o mesmo telefone nunca teria uma linha nos dois lados da união.
//
// status vem null só pra linhas 'ane' ainda sem decisão (nem promovidas a cliente
// Alle, nem reprovadas) — é o que a tabela usa pra decidir entre mostrar os botões de
// ação (status null + academia definida) ou o badge de status (sempre o caso das
// linhas 'manual', que já SÃO um registro de clientes_alle). 'reprovado' do lado
// 'ane' vem de conversions.status (coluna própria — ver migration 0020) em vez de um
// clientes_alle: reprovar não devia depender de academia/nome estarem preenchidos,
// que é o caso de sem-unidade.
//
...
export async function fetchClientesConvertidos(
  ...
  const { rows } = await pool.query<{
    ...
    `select c.id, 'ane' as origem, c.academia_id, a.nome as academia_nome, c.nome, c.telefone,
            case
              when c.cliente_alle_id is not null then 'ativo'
