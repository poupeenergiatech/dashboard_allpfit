...
export default async function PendentesPage() {
  ...
  const [porAcademia, trend, academias, history] = await Promise.all([
    profile ? fetchPendenciasPorAcademia(profile) : Promise.resolve([]),
    profile ? fetchPendenciasTrend(profile) : Promise.resolve([]),
    fetchActiveAcademias(profile),
    profile && canWrite(profile.role) ? fetchPendenciasHistory(profile) : Promise.resolve([]),
  ])
