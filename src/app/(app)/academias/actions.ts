...
export async function createAcademia(formData: FormData) {
  ...
  const conversoesManualAjusteTotal = Number(formData.get('conversoes_manual_ajuste_total') ?? 0)
  ...
  await pool.query(
    'insert into academias (nome, numero_telefone, ativo, total_alunos, conversoes_manual_ajuste_total) values ($1, $2, true, $3, $4)',
    [nome, numeroTelefone || null, totalAlunos, conversoesManualAjusteTotal]
  )
...
export async function updateAcademia(academiaId: string, formData: FormData) {
  ...
  await pool.query(
    'update academias set nome = $1, numero_telefone = $2, total_alunos = $3, conversoes_manual_ajuste_total = $4 where id = $5',
    [nome, numeroTelefone || null, totalAlunos, conversoesManualAjusteTotal, academiaId]
  )
