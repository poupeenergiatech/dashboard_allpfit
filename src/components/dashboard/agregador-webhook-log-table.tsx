            ...
            <tbody>
              {entry.distribuicao.map((d, i) => (
                <tr key={i} className="border-b border-slate-50 last:border-0">
                  <td className="px-3 py-2 text-slate-700">{d.academiaLabel ?? '—'}</td>
                  <td className="px-3 py-2 tabular-nums text-slate-500">{d.telefoneNumero ?? '—'}</td>
                  <td className="px-3 py-2">
                    {d.academiaNome ? (
                      <span className="text-slate-700">{d.academiaNome}</span>
                    ) : (
                      <span className="text-amber-700">não encontrada</span>
                    )}
                  </td>
                  <td className="px-3 py-2">
                    {d.matchedBy === 'nome' && <span className="badge bg-emerald-50 text-emerald-700">nome</span>}
                    {d.matchedBy === 'telefone' && (
                      <span className="badge bg-slate-100 text-slate-600">telefone</span>
                    )}
                    {!d.matchedBy && <span className="text-slate-300">—</span>}
                  </td>
                  <td className="px-3 py-2 text-right tabular-nums text-slate-600">{d.contatosRecebidos}</td>
                  <td className="px-3 py-2 text-right tabular-nums text-emerald-700">{d.contatosInseridos}</td>
                  <td className="px-3 py-2 text-right tabular-nums text-slate-400">{d.contatosIgnorados}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
export function AgregadorWebhookLogTable({ entries }: { entries: AgregadorWebhookLogEntry[] }) {
  ...
  if (entries.length === 0) {
    return <div className="card-dashed text-sm text-slate-500">Nenhum payload recebido ainda.</div>
  }
  ...
  return (
    <div className="space-y-3">
      ...
      {filtered.length === 0 ? (
        <div className="card-dashed text-sm text-slate-500">Nenhum payload encontrado pra esse filtro.</div>
      ) : (
              ...
              {filtered.map((entry) => {
                ...
                return (
                  <Fragment key={entry.id}>
                    <tr className="border-b border-slate-50 align-top transition last:border-0 hover:bg-slate-50/70">
                      ...
                      <td className="px-4 py-3 max-w-[220px] text-xs text-slate-500">
