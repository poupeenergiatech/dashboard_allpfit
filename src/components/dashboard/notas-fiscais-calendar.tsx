...
function NotaFiscalChip({
  ...
  if (entry) {
    return (
      <a
        ...
        className={`group relative block rounded-lg px-2.5 py-1.5 pr-7 ${palette.filled}`}
      >
        ...
        {canManage && (
          <button
            ...
            className="absolute right-1 top-1 flex h-5 w-5 items-center justify-center rounded-md text-current opacity-0 hover:bg-black/5 disabled:opacity-40 group-hover:opacity-70 dark:hover:bg-white/10"
          >
  ...
  if (canManage) {
    return (
      <button
        ...
        className={`rounded-lg border border-dashed px-2.5 py-1.5 text-left text-xs font-semibold transition ${palette.dashed}`}
      >
        + {tipoLabel}
      </button>
    )
  ...
  return (
    <div className="rounded-lg px-2.5 py-1.5 text-xs text-slate-400 dark:text-slate-500">{tipoLabel} — sem nota</div>
  )
}
